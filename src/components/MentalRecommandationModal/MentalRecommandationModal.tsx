...
const MentalRecommandationModal: React.FC<MentalRecommandationModalProps> = ({
  ...
  const data =
    mentalRecommendationsMap[scenario] ||
    ...
    mood: (
      ...
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M15.182 15.182a4.5 4.5 0 01-6.364 0M21 12a9 9 0 11-18 0 9 9 0 0118 0zM9.75 9.75c0 .414-.168.75-.375.75S9 10.164 9 9.75 9.168 9 9.375 9s.375.336.375.75zm-.375 0h.008v.008H9.375V9.75zm5.625 0c0 .414-.168.75-.375.75s-.375-.336-.375-.75.168-.75.375-.75.375.336.375.75zm-.375 0h.008v.008H14.625V9.75z"
        />
      </svg>
    ),
    recommendation: (
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="rounded-2xl p-8 w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl bg-gradient-to-b from-gray-800 to-gray-900">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sections
            .filter(
              (section) => section.value !== undefined && section.value !== null
            )
            .map((section, index) => (
              <div
                key={section.key}
                className={`bg-white/10 rounded-xl p-6 shadow-lg ${
                  section.key === "followUp"
                    ? "md:col-span-2 bg-pink-500/20 border-l-4 border-pink-400"
                    : ""
                }`}
              >
